...
class MigrateData
{
    ...
    {
        $this->loadedData = [];
        $texts = Db::getInstance()->executeS('SELECT i.`id_shop`, il.`id_lang`, il.`text` FROM `' . _DB_PREFIX_ . 'info_new` i
            INNER JOIN `' . _DB_PREFIX_ . 'info_lang_new` il ON il.`id_info` = i.`id_info`'
        );
